...
class AppHeader extends HTMLElement {
    connectedCallback() {
        this.render();
        this._unsubs = [
            store.on('theme-changed', (e) => this.updateThemeUI(e.detail.theme))
        ];

        this.addEventListener('click', (e) => {
            const toggle = e.target.closest('.theme-toggle');
            if (toggle) {
                this.querySelector('.theme-dropdown').classList.toggle('show');
                return;
            }

            const option = e.target.closest('.theme-option');
            if (option) {
                store.setTheme(option.dataset.theme);
                this.querySelector('.theme-dropdown').classList.remove('show');
                return;
            }
        });

        document.addEventListener('click', (e) => {
            const selector = this.querySelector('.theme-selector');
            ...
            <div class="header">
                <div class="header-content">
                    ...
                    <div class="theme-selector">
                        <button class="theme-toggle" aria-label="Select theme">
                        ...
                        <div class="theme-dropdown">
                            ...
                            <button class="theme-option" data-theme="xp">
                                <span class="theme-option-icon">\uD83E\uDE9F</span> Windows XP
                            </button>
                            <button class="theme-option" data-theme="aero">
                                <span class="theme-option-icon">\u2728</span> Vista Aero
                            </button>
                            <button class="theme-option" data-theme="lcars">
                                <span class="theme-option-icon">\uD83D\uDD96</span> LCARS
                            </button>
                        </div>
                    </div>
                </div>
